# Hi, I'm Trevor 👋

I'm a **Software Engineer** and **Computer Science major graduating May 2026** with a **Mathematics minor**, interested in building **scalable, secure, and impactful software systems**. My background spans full-stack development, backend systems, and applied machine learning.

---

## 🚀 About Me
- 🎓 Computer Science (B.S.) with Mathematics minor
- 💼 Software Engineer Intern experience at **HubSpot**, **ServiceNow**, and **Intuit**
- 🧠 Strong foundation in **data structures, algorithms, and system design**
- 🌍 Open to **New Grad** and **Internship** opportunities

---

## 🛠️ Technical Skills

**Languages**
- Java, Python, JavaScript, TypeScript, SQL

**Frameworks & Tools**
- React, Node.js, Express, Next.js
- REST APIs, Fastify
- PostgreSQL, MySQL, MongoDB
- Docker, Git, CI/CD

**Concepts**
- Full-Stack Development
- Backend & API Design
- Distributed Systems (foundations)
- Machine Learning (foundations)

---

## 📌 Featured Projects

### ✈️ Smart Travel App
Full-stack trip-planning platform with a Next.js web client and a TypeScript API backed by Supabase. Includes collaborative itineraries, AI-assisted planning, timelines, and chat.
- Tech: Next.js, TypeScript, Fastify, Supabase, Docker  
- 🔗 [GitHub](https://github.com/Trevorkaaya21/smart-travel)

### 🎬 Movie Match App
A full-stack application inspired by swipe-based matching to help users decide what to watch.
- Tech: JavaScript, React, Node.js, MongoDB  
- 🔗 [GitHub](https://github.com/skishra/movie-match)

### 🏫 University Management System
Java/MySQL-based system for managing students, courses, and enrollment workflows.
- Tech: Java, MySQL, OOP

---

## 🎯 What I’m Looking For
- Software Engineering Internships
- New Grad Software Engineer roles
- Opportunities to work on **production systems** with real-world impact

---

## 📫 Let’s Connect
- 🌐 Portfolio: https://trevorkkaayaportfolio.netlify.app
- 💼 LinkedIn: https://www.linkedin.com/in/trevor-kkaaya
- 📧 Email: trevor.kkaaya@gmail.com

